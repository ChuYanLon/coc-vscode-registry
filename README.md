...
| `prebuilt` | ❌ | Download pre-compiled server from VS Code marketplace instead of building from source. For `kind: "module"` servers that need compilation (TypeScript, wasm, etc.). `{ type: "vsix", publisher: "...", extension: "...", version: "...", serverPaths: ["server"] }` |
